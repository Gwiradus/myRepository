...
for i in range(iterations):
    ...
    for ep, aFleet in enumerate(myFleets):
        ...
        events = np.around(np.sort(np.concatenate((time, myFleet.arrival_times, myFleet.departure_times))), decimals=2)
...
# Saving the objects:
# with open('fqiETResults.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
